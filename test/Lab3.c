...
long MaxJitter,MaxJitter2;    // largest time difference between interrupt trigger and running thread
long MinJitter,MinJitter2;    // smallest time difference between interrupt trigger and running thread
...
semaphore_t Ready;       // used for mutual exclusion
...
semaphore_t Free;       // used for mutual exclusion
...
semaphore_t Readyc;        // set in background
int Lost;
...
// no ADC serial port or oLED output
// tests the spinlock semaphores, tests Sleep and Kill
semaphore_t Readyd;        // set in background
...
// 	UINT thisTime = OS_Time();
// 	int period = (thisTime - tasks[i].lastTime)/50;
// 	int jitter = tasks[i].period - period;
// 	if(thisTime < tasks[i].lastTime) {
// 		period = (((UINT)-1)-tasks[i].lastTime)/50+thisTime/50;
...
// select interrupts, 
// down no interrupts
semaphore_t s;            // test of this counting semaphore
